...
int main(){
	...
	ofstream sMatch("Match & Roll History.txt", ios::app);
	sMatch<<"---Match Start---"<<"\n";
	sMatch.close();
	...
	ofstream hash1("Match & Roll History.txt", ios::app);
	hash1<< "Hash P2 #" <<cChamp<<"-> "<< p1champ<<"\n";
	hash1.close();
	...
	ofstream hash2("Match & Roll History.txt", ios::app);
	hash2<<"Hash P2 #"<<pChamp<<"-> "<< p2champ<<"\n";
	hash2.close();
  ...
  box(field2, 0, 0);
    ...
    if(cHp==-999 || pHp==-999 ){
			...
			cout << "Player 1 Wins";
			ofstream p1Win("Match & Roll History.txt", ios::app);
			p1Win<<"Player 1 Wins"<<"\n";
			p1Win.close();
			goto end;
		}else if(pHp>0){
			cout << "Player 2 Wins";
			ofstream p2Win("Match & Roll History.txt", ios::app);
			p2Win<<"Player 2 Wins"<<"\n";
			p2Win.close();
			goto end;
		}
	end:
	ofstream eMatch("Match & Roll History.txt", ios::app);
	eMatch<<"---Match End---"<<"\n\n\n";
	eMatch.close();
	return 0;	
    key = getch();
